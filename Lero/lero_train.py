...
def training_pairwise(tuning_model_path, model_name, training_data_file, pretrain=False):
    X1, X2 = _load_pairwise_plans(training_data_file) # 构建n(n-1)/2对比较数据
...
######################## new
def get_feaure(model_path, data_path, batch_size=64):
    ...
    features1 = torch.concatenate(features1)
    features2 = torch.concatenate(features2)
